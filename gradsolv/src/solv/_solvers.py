...
from src._error_handling import error_handle
from src.precond._preconditioners import grad_preconditioner
import src.solv._auxiliary_routines as _aux
...
class grad_solver(object):
    ...
    def __solve(self, matrix: ndarray, rhs: ndarray, err_handle: error_handle, tol: float, max_it: int, include_relres_vec=False) -> grad_solver_result:
    ...
    def solve(self, matrix: ndarray, rhs: ndarray, err_handle: error_handle, tol: float, max_it: int, include_relres_vec=False) -> grad_solver_result:
        ...
        if error_occured:
            ...
            return self.__solve(self, matrix, err_handle, tol, max_it, include_relres_vec)
...
class steepest_descend_solver(grad_solver):
    ...
    def __solve(self, matrix: ndarray, rhs: ndarray, err_handle: error_handle, tol: float, max_it: int, include_relres_vec=False) -> grad_solver_result:
        ...
        x = zeros(shape = n)
        r = rhs - (matrix @ x)
        z = self.__preconditioner(matrix, r) if self.__preconditioner else r
        ...
        relres_vec = array(relres_vec) if relres_vec else None
